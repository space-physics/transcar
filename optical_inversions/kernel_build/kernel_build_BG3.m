...
for ie=1:length(idlist)
    fprintf('\nKERNEL_BUILD_BG3.M --> loading files for simulation:  %s', idlist{ie});
    
    %load data for this beam
    load([idlist{ie},'/plasma.mat']);
    load([idlist{ie},'/dat.mat']);
    [VER,z,t,lambda,IDs,b,diffnumflux_interp,E_interp]=VER_calc(idlist{ie},'excrates.mat');
    ...
    phiN(ie)=max(precchr(10:le,ie))/2/pi;          %The units here are cm-2 s-1 eV-1 sr-1
end
fprintf('\n');
...
%SAVE RESULTS IN PWD
save(['kernel_',datelab,'.mat'],'Mp','Mb','lambda','energies','z','zop','phiN','datelab','E','IDs');
rmpath ~/matlabshared;
