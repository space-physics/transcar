...
c	implicit none
        ...
        integer ndegmax,mdegmax
        parameter(ndegmax=20,mdegmax=20)
        integer ndeg,mdeg
	integer iyddeb,iydfin,iyd
	real temps,tempsdeb,tempsfin
	logical flgini
	real tu,Energie,fluxE,Flux_elec_int,lonmlt
	real*8 latequi,Lmin,Lmax,DL,LM,deg2rad,coeffc,alfa
	parameter (deg2rad=.01745329251994,coeffc=6.)
	real*8 fx,d11,d12,d21,d22,det
	complex*16 phienerg(0:mdegmax,0:ndegmax)
	complex*16 phiflux(0:mdegmax,0:ndegmax)
	complex*16 expphi(0:mdegmax),xcomp,ejphi
	...
	if (flgini) then
     ...
     &                       //'dir.source/dir.cine/varenerg.dat',
     &		form='formatted',status='old',iostat=ierr,err=999)
	  open(78,file=chemin(1:lpath)
     &                       //'dir.source/dir.cine/varfluxe.dat',
     &		form='formatted',status='old',iostat=ierr,err=999)
	endif
...
c 	      Eo = (Flux d'energie/2*Flux de particules)
	...
	if (iyd.lt.1900000) xt=xt+1900000.
     ...
     &                 ndeg,mdeg,Lmin,Lmax,latequi,ddp
	    ...
	    read(77,*) ((phienerg(m,n),m=0,mdeg),n=0,ndeg)
	    read(78,*) ((phiflux(m,n),m=0,mdeg),n=0,ndeg)
	  enddo
	  close(77)
	  close(78)
