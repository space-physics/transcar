...
c	implicit none
	...
	complex*16 phicourant(0:mdegmax,0:ndegmax)
	complex*16 expphi(0:mdegmax),xcomp,ejphi
	real*8 x,pn,dp,xt,xtd,xtf
	integer ikp,m,n
...
999	continue
	...
	if (iyd.lt.1900000) xt=xt+1900000.
     ...
     &                 ndeg,mdeg,Lmin,Lmax,latequi,ddp
	    ...
	    read(67,*) ((phicourant(m,n),m=0,mdeg),n=0,ndeg)
	  enddo
	  close(67)
	  ...
	  Jsup=0.
	    ...
	    do n=0,ndeg
	      ...
	      do m=0,mdeg
	        Jsup=Jsup-real(phicourant(m,n)*pn*expphi(m))
	      enddo
	    enddo
	    ...
	    do n=0,ndeg
	      ...
	      do m=0,mdeg
	        ...
	        Fnm=(anm*(expcdx-1.)+bnm)*expmdx*expphi(m)
                Jsup=Jsup-real(phicourant(m,n)*Fnm)
	      enddo
	    enddo
