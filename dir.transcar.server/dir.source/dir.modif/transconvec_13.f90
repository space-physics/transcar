...
c	date mise au format YYYYDDD

c	if (iyd_ini.lt.100000) then

c	date au format YYDDD
c	  if (iyd_ini/1000.ge.80) then
c	    iyd_ini=1900000+iyd_ini
c	  else
c	    iyd_ini=2000000+iyd_ini
c	  endif

c	elseif (iyd_ini.lt.1000000) then	

c	data au format YYMMDD
c	  call mois_jour(iyd_ini,iday)
c	  if (iyd_ini/10000.lt.80) then
c	    iyd_ini=1900000+iday
c	  else
c	    iyd_ini=2000000+iday
c	  endif

c	elseif (iyd_ini.ge.10000000) then	

c	date au format YYYYMMJJ
c	  call mois_jour(iyd_ini,iday)
c	  iyd_ini=iyd_ini/10000
c	  iyd_ini=1000*iyd_ini+iday
...
cc      NO continuity equation
     ...
     &      +1.5e-11*Nn(i)*No2(i)/N_0*exp(-3600./Tn(i))
     &           )                  *t0
     ...
     &    2.e-11*N3new(i)*N_0+
     &      ak4*N1new(i)*N_0
     &    +(8.3e-6+6.e-7)+1.5e-12*(Tn(i)**.5)*Nn(i)
     &        )      *t0
...
C[[[    O+ heat flow equation resolution
     ...
     &                          )
     &           -2.5*T1new(i)*N1new(i)*(
     &                         (nujN2(i)+nujO2(i)+nujO(i))*(Un(i)/Cj0)
     &               -9.6*Njqj0*
     ...
     &                  )               )
          ...
          D7(i)=-(nujN2(i)*.34+nujO2(i)*.27+nujO(i)*.725
...
C[[[    N+ heat flow equation resolution
     ...
     &                           )
     &          -2.5*T3new(i)*N3new(i)*(
     &                        (nunN2(i)+nunO2(i)+nunO(i))*(Un(i)/Cn0)
     &           -8.4*Nnqn0*(
     ...
     &                                 )
          ...
          D7(i)=-( nunN2(i)*.27+nunO2(i)*.2+nunO(i)*.62+nuni(i)*2.62
...
C[[[    Electron energy and heat flow equation resolution
     ...
     &                                 )
          ...
          D7q(i)=.2*(nueN2(i)+nueO2(i)+nueO(i)+nueH(i)
...
c	fin de modif
	...
	nu_omega=(nueN2(i)+nueO2(i)+nueO(i)+nueH(i))/omega(i)*ome
	coef_conv=1./(1.+nu_omega**2)
...
c	temperature parallele
	...
	nu_omega=(nuiN2(i)+nuiO2(i)+nuiO(i)+nuiH(i))/omega(i)*omi
	coef_conv=1./(1.+nu_omega**2)
...
c	temperature parallele
	...
	nu_omega=(nunN2(i)+nunO2(i)+nunO(i))/omega(i)*omn
	coef_conv=1./(1.+nu_omega**2)
...
C[[[    heavy ions momentum equation resolution
     ...
     &            /(Tmnew(i))**1.5				/nu_0
          nukl(i)=0.18*N6new(i)
     &            /(Tmnew(i))**1.5				/nu_0
          nuke(i)=1.07e-3*Nenew(i)/nu_0/(Tenew(i))**1.5		/nu_0
     ...
     &            / ((14.*Tmnew(i)+32.*T3new(i))/46)**1.5	/nu_0
          nulk(i)=0.15*N4new(i)/nu_0/(Tmnew(i))**1.5		/nu_0
          nulm(i)=0.16*N5new(i)/nu_0/(Tmnew(i))**1.5		/nu_0
          null(i)=0.16*N6new(i)/nu_0/(Tmnew(i))**1.5		/nu_0
          nule(i)=9.347e-4*Nenew(i)/nu_0/(Tenew(i))**1.5	/nu_0
          numj(i)=0.14*N1new(i)
...
C-----------------------------------------------------------------------
        real function ylimd(x1,y,extra,nx)

        real x1,y(nx),extra(5)

C extra(1)=Radn(nx-2) C extra(2)=Radn(nx-1) C extra(3)=Radn(nx ) C extra(4)=xb C extra(5)=sig2

        ymoy=(y(nx-2)+y(nx-1)+y(nx))/3.
...
C-----------------------------------------------------------------------
        real function bclimvd(x1,y,extra,nx)
...
C-----------------------------------------------------------------------
        real function bclimd(x1,y,extra,nx)
...
C-----------------------------------------------------------------------
        real function bclimld(x1,y,extra,nx)
...
C       extra(5)=sig2
        ...
        ymoy=(log(y(nx-2))+log(y(nx-1))+log(y(nx)))/3.
        xymoy=(extra(1)*log(y(nx-2))
     &          +extra(2)*log(y(nx-1))
     &          +extra(3)*log(y(nx)))/3.

        bclimld=ymoy
     &         -.5*abs((xymoy-extra(4)*ymoy)*(x1-extra(4))/extra(5))
        bclimld=exp(bclimld)/y(nx)
        bclimld=min(bclimld,.85)
