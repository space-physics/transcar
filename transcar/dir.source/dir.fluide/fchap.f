...
c
	parameter (nlaguer=15 , ngauss=8 , nmax=31,nesp=7)
	real zi(nmax),lati(nmax),loni(nmax),wi(nmax)
	real zj(nlaguer),wj(nlaguer)
	integer ni,nj
	real lat, lon,ap(7),dn(8),tn(2),f107(3),tu,sec
	real chap1(nesp),chap2(nesp),chapesp(nesp)
        real indesp(nesp)
 	data pideg/1.745329252e-2/
	data re/6356.766/
	...
	subroutine grille(z,lat,lon,tu,chi,decli,zi,lati,loni,wi,ni,
     &				zj,wj,nj)
	parameter (nlaguer=15 , ngauss=16 , nmax=31)
	real lat,lon,latref,lonref
	real zi(nmax),lati(nmax),loni(nmax),wi(nmax)
	real zj(nlaguer),wj(nlaguer)
	real*8 xl(nlaguer),wl(nlaguer),xg(ngauss),wg(ngauss)
	real absc(nmax)
	integer ni,nj
	data pis2,re/1.5707963268,6356.766/
	data rad2deg,deg2rad/57.2957795132,.01745329252/
