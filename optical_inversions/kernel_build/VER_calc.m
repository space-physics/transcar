function [VER,z,t,lambda,specID,spectype,blambda,diffnumflux,E]=VER_calc(datadir,infile)
...
r=zop./sind(mean(dipangle))*1e5;          %convert altitudes (km) to distance (cm) along flux tube
z=zop(:,1);
lz=numel(z);
t=timeop;
lt=numel(t);
...
fprintf('\nVER_CALC.M --> Computing metastable emissions.');
Ams=[1.215, 0.076, 7.1e-3, 2.2e-3, 0.173, 0.047];                     %Einstein coeff. (1/s)
lambdams=[557.7; 297.2; 630.0; 636.4; 732; 247.0];                    %wavelength (nm)
ll=numel(lambdams);
specID=[specID,'O(^1S) 557.7','O(^1S) 297.2','O(^1D) 630.0','O(^1D) 636.4', ...
    'O^+(^2P) 732-733','O^+(^2P) 247.0'];
spectype=[spectype,ones(1,ll)];

VERtmp=zeros(lz,lt,ll);
...
lambda=lambdams;
blambda=blambdams;
VER=cat(3,VER,VERtmp);


%PROMPT ATOMIC EMISSIONS
fprintf('\nVER_CALC.M --> Computing prompt atomic emissions.');
lambdapa=[844.6; 777.4];
ll=numel(lambdapa);
specID=[specID,'O(3p^3P) 844.6','O(3p^5P)'];
spectype=[spectype,2*ones(1,ll)];
...
fprintf('\nVER_CALC.M --> Computing N2+ 1NG emissions.');
...
scalevec=scalemat(:);
ll=numel(lambda1NGvec);
[lvp,lvpp]=size(A1NG);
for ivpp=1:lvpp
    for ivp=1:lvp
        specname=sprintf('N_2 1N (%d-%d) %5.1f',ivp-1,ivpp-1,lambda1NG(ivp,ivpp));
        specID=[specID,specname];
    end
end
spectype=[spectype,3*ones(1,ll)];

%Compute vibrational bands
VERtmp=zeros(lz,lt,ll);
for k=1:length(scalevec)
    VERtmp(:,:,k)=p1ng*scalevec(k);
    [integ,dr]=intrap(VERtmp(:,:,k),r);                                 %integrate along flux tube
    [nrow,ncol]=size(integ);
...
%N2+ Meinel band
fprintf('\nVER_CALC.M --> Computing N2+ Meinel emissions.');
...
scalevec=scalemat(:);
ll=numel(lambdaMeinvec);
[lvp,lvpp]=size(AMein);
for ivpp=1:lvpp
    for ivp=1:lvp
        specname=sprintf('N_2^+ M (%d-%d) %5.1f',ivp-1,ivpp-1,lambdaMein(ivp,ivpp));
        specID=[specID,specname];
    end
end
spectype=[spectype,4*ones(1,ll)];

%Compute vibrational bands
VERtmp=zeros(lz,lt,ll);
for k=1:length(scalevec)
    VERtmp(:,:,k)=pmein*scalevec(k);
    [integ,dr]=intrap(VERtmp(:,:,k),r);                                 %integrate along flux tube
    [nrow,ncol]=size(integ);
...
fprintf('\nVER_CALC.M --> Computing N2 2PG emissions.');
...
scalevec=scalemat(:);
ll=numel(lambda2PGvec);
[lvp,lvpp]=size(A2PG);
for ivpp=1:lvpp
    for ivp=1:lvp
        specname=sprintf('N_2 2P (%d-%d) %5.1f',ivp-1,ivpp-1,lambda2PG(ivp,ivpp));
        specID=[specID,specname];
    end
end
spectype=[spectype,5*ones(1,ll)];

%Compute vibrational bands
p2PGvec=[];
VERtmp=zeros(lz,lt,ll);
for k=1:length(scalevec)
    VERtmp(:,:,k)=p2pg*scalevec(k);
    [integ,dr]=intrap(VERtmp(:,:,k),r);                                 %integrate along flux tube
    [nrow,ncol]=size(integ);
...
scalevec=scalemat(:);
ll=numel(lambda1PGvec);
[lvp,lvpp]=size(A1PG);
for ivpp=1:lvpp
    for ivp=1:lvp
        specname=sprintf('N_2 1P (%d-%d) %5.1f',ivp-1,ivpp-1,lambda1PG(ivp,ivpp));
        specID=[specID,specname];
    end
end
spectype=[spectype,6*ones(1,ll)];

%Compute vibrational bands for 1PG
VERtmp=zeros(lz,lt,ll);
for k=1:length(scalevec)
    VERtmp(:,:,k)=N01pg*scalevec(k);
    [integ,dr]=intrap(VERtmp(:,:,k),r);                                 %integrate along flux tube
    [nrow,ncol]=size(integ);
...
fprintf('\nVER_CALC.M --> Cleaning up data arrays.');
inds=find(lambda~=0);
blambda=blambda(inds,:);
lambda=lambda(inds);
VER=VER(:,:,inds);
specID=specID(inds);
spectype=spectype(inds);
...
%MOST DENSE ENERGY GRID USED IN SIM
fprintf('\nVER_CALC.M --> Interpolating fluxes for plotting.\n');
lE=max(nen);
eflag=0;
iset=0;
while ~eflag
    iset=iset+1;
    vnamee=['e',num2str(iset)];
    Etmp=eval(vnamee);
    [lEtmp,lttmp]=size(Etmp);
    if lEtmp==lE
        E=Etmp(:,1);
        eflag=1;
    end
...
%INTERPOLATE ONTO DENSE GRID FOR USER
eflag=0;
iset=0;
diffnumflux=zeros(lE,lt);
ltprev=0;
lttmp=0;
while ~eflag
    iset=iset+1;
    vnamee=['e',num2str(iset)];
    vnamef=['fluxdown',num2str(iset)];
    if exist(vnamee)
        Etmp=eval(vnamee);
        Ftmp=eval(vnamef);
        ltprev=ltprev+lttmp;
        [lEtmp,lttmp]=size(Etmp);
        for k=1:lttmp
            diffnumflux(:,k+ltprev)=interpolate(Ftmp,Etmp,E,'hold','loglin')/2/pi;
        end
    else
        eflag=1;
    end
