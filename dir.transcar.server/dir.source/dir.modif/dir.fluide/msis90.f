	subroutine msis90(iyd,sec,alt,glat,glong,f_107,ap,mass,d,t,w)

	implicit none
	
	integer i,iyd,mass
	real sec,alt,glat,glong,f_107(3),ap(7),d(8),t(2),w(2)
	real F107,F107A,STL
...
c
     	 subroutine atmntr(modatmos,neutspe,dayno,nan,Apind,afmo,afh,
     .           alat,along,UT,z,nalt,sortie,dens,fcdens,tinf,tneutre,
     . 		 fctemp,xmasdens,impress)
c
c    Calcule l'atm neutre avec iteration eventuelle pour imposer tinf
c
C     INPUT for atmntr:
c     	modatmos: Sert a choisir l'iteration pour imposer Tinf.
c 		 = 1 si tinf calcule par msis
c                = 2 si iterations sur f107
c                = 3 si iterations sur le flux, ET sur Ap. 
c                 (jl,janv 88, sept 91)
c  	neutspe  = Nbre d'especes considerees.
c 	dayno = Day Number (ddd)
c 	nan = annee (sous forme 19yy ou yy indifferement)
c       Apind  = indice Ap qui sera rentre dans le tableau AP de MSIS.
c	afmo = f107 moyen
c 	afh = f107 
c 	alat = latitude
c 	along = longitude
c 	UT = temps universel en heures decimales.
c 	z = tableau d'altitudes sur lesquelles calculer l'atm.
c 	nalt = nbre d'altitudes dans le tableau z.
c 	sortie = .true. si on veut une impression dans le fic. impress.
c       fcdens : Tableau de facteurs multiplicatifs des densites, de 
c 		dimension neutspe.
c       fctemp : Facteur multiplicatif de la temperature.
c 	
c     OUTPUT DE ATMNTR
c 	dens   = tableau de densites des especes /cm3 a chaque altitude.
c 		1=N2 2=O2 3=O 4=H 5=He 6=N 7=A
c 	tneutre= tableau de dimension nalt des temperatures neutres.
c 	xmasdens= total mass density(gm/cm3)
c      
c     INPUT/OUTPUT DE ATMNTR
c        tinf = temperature exospherique desiree
c             = 1 si calculee par MSIS
c
c     INPUTS DE MSIS:
...
C     OUTPUT: 
c        
C        D(1) - HE NUMBER DENSITY(CM-3)    = dens(5,ialt)
C        D(2) - O NUMBER DENSITY(CM-3)     = dens(3,ialt)
C        D(3) - N2 NUMBER DENSITY(CM-3)    = dens(1,ialt)
C        D(4) - O2 NUMBER DENSITY(CM-3)    = dens(2,ialt)
C        D(5) - AR NUMBER DENSITY(CM-3)    = dens(7,ialt)
C        D(6) - TOTAL MASS DENSITY(GM/CM3) = xmasdens
C        D(7) - H NUMBER DENSITY(CM-3)     = dens(4,ialt)
C        D(8) - N NUMBER DENSITY(CM-3)     = dens(6,ialt)
C        T(1) - EXOSPHERIC TEMPERATURE     = tinf
C        T(2) - TEMPERATURE AT ALT         = tneutre
c
c 	ATTENTION ! NO n'est pas une sortie de MSIS. En attendant,
c 	l'estimation de NO est donnee par la fonction DENSNO, donnee
c 	par Charley Taieb (sources ?...).
c
      dimension z(nalt),tneutre(nalt),dens(8,nalt),fcdens(8)
      dimension xmasdens(nalt)
      logical sortie
      DIMENSION D(8),T(2)
c
 	if(nan.lt.100)then
          anjour=float(nan)*1000.+dayno 
 	else
          anjour=(float(nan)-1900)*1000.+dayno 
 	endif
        iyd = ifix(anjour)
c 	sec est le temps en secondes
c 	stl est le temps solaire en heures
c 	Formule pour un temps local:
c       sec=amod(hr-along*12./180+24.,24.)*3600.
c       stl=hr + ifix(along/15.)-along/15.
c 	Formule pour UT :
 	SEC=UT*3600.
 	IF(SEC.GT.86399.) SEC=SEC-86399.
 	STL=UT+along*24/360.
c 	Pour les iterations, on appelle msis a une alt. bidon > 85 km 
c 	pour les versions avant msis90.
        altbid=1000.
c 	On conserve les valeurs initiales.
c 	Les iterations se feront directement sur Apind, afmo et afh
        f107a=afmo
        f107=afh
  	aap=Apind
        tinf1=tinf
c
 	if(tinf.eq.1.)then
          call gtd6(iyd,sec,altbid,alat,along,stl,afmo,afh,Apind,48,d,t)
  	  tinf=t(1)
  	elseif (tinf.ne.1.)then
 	  if (modatmos.eq.2) then
c 	    Iteration sur f107 seul.
            ttinf=tinf
            aaf=afmo
            afh=afmo
            aafi=0.
...
1000        continue
            afmo=aaf
            afh=aaf
c           write (6,2000)iterat,afmo,ttc,ttinf
2000        format ('iterat, afmo , ttc,t desiree ',i5,5f10.2)
            iterat=iterat+1
c
 	    t(1)=0.
 	    t(2)=0.
            call gtd6(iyd,sec,altbid,alat,along,stl,afmo,afh,Apind,
     .			48,d,t)
c
            ttc=t(1)
            if((ttc.gt.ttinfl).and.(ttc.lt.ttinfh)) go to 1010
            IF (aaf.LT.1E-36)GO TO 1010
c
            if (iterat.gt.50)then
 	      write(6,*)'nombre d''iterations sur f107 > 50'
 	      write(6,*)'arret du programme'
 	      stop
 	    endif
c
            if (ttc.lt.ttinf) then
              go to 1020
            else
              goto 1030
            endif
...
c           write (6,2000)iterat,afmo,ttc,ttinf
c
	  else if (modatmos.eq.3)then
c           Iteration sur f107 ET Ap 
            tdesir=tinf
            dal=0.
 	    dah=2.*Apind
 	    dfl=0.
	    ...
	    iterf=0 
c
c	    write(6,*)'f107 reel,Ap reel,Tinfinie desiree:',
c    .  	   f107,aap,tdesir
c 	    write(6,*)
c
210         call gtd6(iyd,sec,altbid,alat,along,stl,afmo,afmo,Apind,
     .		    48,d,t)
c
c	    write(6,*)'f107 calc,Ap calc,Tinfinie calc:',
c    .			afmo,Apind,t(1),'  [A'
 	    if(t(1).gt.tinfh)then
              call gtd6(iyd,sec,altbid,alat,along,stl,afmo,afmo,dal,
     .		    48,d,t)
c	      write(6,*)'f107 calc,Ap calc,Tinfinie calc:',
c    .			afmo,dal,t(1)
 	      if(t(1).gt.tinfh)then
c 	        Modifier f107,car meme avec Ap=0 on reste trop haut
                write(6,*)' il faut modifier f107,car meme avec Ap=',dal
                write(6,*)' on reste trop haut'
 	        iflag=4
 	      else
c 	        on peut iterer sur Ap
  	        write(6,*)'on peut iterer sur Ap'
 	        iflag=2
 	      endif
 	    elseif(t(1).lt.tinfl)then
              call gtd6(iyd,sec,altbid,alat,along,stl,afmo,afmo,dah,
     .		    48,d,t)
c	      write(6,*)'f107 calc,Ap calc,Tinfinie calc:'
c	      write(6,*)afmo,dah,t(1)
 	      if(t(1).lt.tinfl)then
c 	        Modifier f107,car meme avec 2 Ap on reste trop bas
                write(6,*)' il faut modifier f107,car meme avec ',dah
                write(6,*)' on reste trop bas'
 	        iflag=3
 	      else
c 	        on peut iterer sur Ap
c 	        write(6,*)'on peut iterer sur Ap'
 	        iflag=1
 	      endif
 	    else
c 	      on est directement bon
c 	      write(6,*)'on est directement bon'
 	      iflag=0
 	      go to 230
 	    endif
...
c 	      il faut diminuer f107.On le fait doucement:
 	      iterf=iterf+1
 	      if (iterf.gt.50)then
 	        iflag=5
 	        go to 230
 	      endif
 	      afmo=afmo-(afmo-dfl)/10.
 	      dfh=dfl+2.*afmo
 	      go to 210
  	    elseif(iflag.eq.3)then
c 	      il faut augmenter f107.On le fait doucement:
 	      iterf=iterf+1
 	      if (iterf.gt.50)then
 	        iflag=5
 	        go to 230
 	      endif
 	      afmo=afmo+(dfh-afmo)/10.
 	      dfl=dfh-2.*afmo
 	      go to 210
 	    elseif(iflag.eq.2)then
c 	      if faut diminuer Ap
 	      itera=itera+1
 	      if (itera.gt.50)then
 	        iflag=6
 	        go to 230
 	      ...
 	      dah=Apind
 	      Apind=Apind-(Apind-dal)/2.
              call gtd6(iyd,sec,altbid,alat,along,stl,afmo,afmo,Apind,
     .		    48,d,t)
c	      write(6,*)'f107 calc,Ap calc,Tinfinie calc:',
c    .			afmo,Apind,t(1)
 	      if(t(1).lt.tinfl)then
 	        iflag=1
 	      elseif(t(1).gt.tinfh)then
 	        iflag=2
 	      else
 	        iflag=0
 	      endif
...
c 	      if faut augmenter Ap
 	      itera=itera+1
 	      if (itera.gt.50)then
 	        write(6,*)'nombre d''iterations sur Ap > 50'
 	        write(6,*)'arret du programme'
 	        stop
 	      endif
c
 	      dal=Apind
 	      Apind=Apind+(dah-Apind)/2.
              call gtd6(iyd,sec,altbid,alat,along,stl,afmo,afmo,Apind,
     .		    48,d,t)
c	      write(6,*)'f107 calc,Ap calc,Tinfinie calc:',
c    .			afmo,Apind,t(1)
...
1042      continue
          yd=anjour
c 	
 	endif		! Fin de la condition sur tinf.
c
c----fin de boucles
c
C        D(1) - HE NUMBER DENSITY(CM-3)    = dens(5,ialt)
...
C        D(8) - N NUMBER DENSITY(CM-3)     = dens(6,ialt)
c
c             - N0 NUMBER DENSITY(CM-3)     = dens(8,ialt)
c        
 	do ialt=1,nalt
 	  alt = z(ialt)
 	  call gtd6(iyd,sec,alt,alat,along,stl,afmo,afh,Apind,48,
     .		      d,t)
          tneutre(ialt)=t(2)*fctemp
	  dens(1,ialt)=d(3)*fcdens(1)				   !N2
	  if(neutspe.ge.2)dens(2,ialt)=d(4)*fcdens(2)		   !O2
	  if(neutspe.ge.3)dens(3,ialt)=d(2)*fcdens(3)		   !O
	  if(neutspe.ge.4)dens(4,ialt)=d(7)*fcdens(4)		   !H
	  if(neutspe.ge.5)dens(5,ialt)=d(1)*fcdens(5)		   !He
	  if(neutspe.ge.6)dens(6,ialt)=d(8)*fcdens(6)		   !N 
	  if(neutspe.ge.7)dens(7,ialt)=d(5)*fcdens(7)		   !Ar
	  if(neutspe.ge.8)dens(8,ialt)=densno(alt,stl)*fcdens(8)   !NO
          xmasdens(ialt)=d(6)
 	enddo
c
c      let the results be known
c
2010    format ('atmosphere neutre, modele MSIS 86',/,
     .         f10.2, 20x, 'tinf')
2020    format (' lat=' ,f7.2,3x,'long=',f7.2,3x,'ap=',f7.2)
2023    format (' an*1000+jour=',f10.1,3x,'annee=',i7)
2030    format('flux moyen ',f7.2,3x,'flux de la veille ',f7.2)
2035    format('flux moyen ',f7.2,3x,'flux de la veille=flux moyen= ',
     .         f7.2)
2040    format('iterations sur f107.',i2,'iterations',/)
2045     format(/,' iterations sur f107 et ap.',/,
     .       i2,'iterations sur f107,',i2,'iterations sur ap')
2050    format (/,' no.   height    temp    mass density  n(n2)     '
...
100     format(1x,'mass', i5, '  not valid')
        if (sortie) then
          write(impress,2010)t(1)
          write(impress,2020)alat,along,Ap
  	  write(impress,2023)anjour,nan
 	  if (tinf1.eq.1.)then
             write(impress,2030)afmo,afh
 	  else  
             write(impress,2035)afmo,afh
 	     if (modatmos.eq.2)then
               write(impress,2040)iterat
 	     elseif (modatmos.eq.3)then
               write(impress,2045)iterf,itera
 	     endif
 	  endif
          write(impress,2050)
	  if(neutspe.le.3)then
	    iwrite=neutspe
	    do 130 i=nalt,1,-1
              write(impress,2060)i,z(i),tneutre(i),xmasdens(i),
     .                          (dens(isp,i),isp=1,iwrite)
130 	    continue
	  else
	    iwrite=3
	    do 140 i=nalt,1,-1
...
140 	    continue
            write(impress,2055)
	    do 150 i=nalt,1,-1
              write(impress,2065)i,z(i),(dens(isp,i),isp=4,neutspe)
150 	    continue
	  endif
        endif
c
...
c **************************************************************** c
      ...
      FUNCTION DENSNO(HA,HR)
c 	
c 	ha = altitude en km
c 	hr = stl de msis (temps solaire en heures)
      DENSNO=0.
      PI=6.283185
      IF(HA.LT.85.) RETURN
      IF(HA.GT.115.) GO TO 10
      DENSNO=9.996037E7-159936.59*(HA-115.)**2 
      BETA=SIN(PI/24.*(HR-12))*DENSNO/3.
      DENSNO=max(0.,DENSNO+BETA)
      RETURN
   10 F1=1.E8*EXP((115.-HA)/30.)
	DENSNO = F1 - 39630.2
      BETA=SIN(PI/24.*(HR-12))*DENSNO/3.
      DENSNO=DENSNO+BETA
      IF(HA.GT.300) THEN
      DENSNO = exp(-0.0485*HA+26.553)
      ENDIF
      RETURN
      END
c
...
Ce       TINF VARIATIONS NOT IMPORTANT BELOW ZA OR ZN1(1)
      IF(ALT.GT.ZN1(1)) THEN
        IF(V2.EQ.1..OR.ALAST.LE.ZN1(1)) THEN
	  TINF=PTM(1)*PT(1)
